
package sudoku.c306.a3;

/**
 * @author Donald
*
* Interface d'une grille de sudoku
*/
public class Grille {

    /**
    * Caractere de case vide.
    */
	public static final char EMPTY = '@';

    /**
    * Caractere possible a mettre dans la grille.
    * pour une grille 9x9 : 1..9
    * pour une grille 16x16: 0..9-a..f
    */
	protected char[] possible = new char[] {'1', '2', '3', '4', '5', '6',
            '7', '8', '9', '0', 'a', 'b', 'c', 'd', 'e', 'f'};

    /**
    * @return largeur/hauteur de la grille
    */
    int getDimension() {
		return 0;
	}

    /**
    * Affecte une valeur dans la grille.
    *
    * @param x position x dans la grille
    * @param y position y dans la grille
    * @param value valeur a mettre dans la case
    * @throws IllegalArgumentException si x ou y sont hors bornes (0-8)
    * @throws IllegalArgumentException si la valeur est interdite aux vues des
    * autres valeurs de la grille
    * @throws IllegalArgumentException si value n'est pas ('1',...,'9')
    */
    void setValue(int x, int y, char value) {
    	//Affecte une valeur dans la grille
	}

    /**
    * Recupere une valeur de la grille.
    *
    * @param x position x dans la grille
    * @param y position y dans la grille
    * @return valeur dans la case x,y
    * @throws IllegalArgumentException si x ou y sont hors bornes (0-8)
    */
    char getValue(int x, int y) {
		return 0;
	}

    /**
    * Test si une grille est terminee.
    *
    * @return true si la grille est complete
    */
    boolean complete() {
		return false;
	}

    /**
    * Test si une valeur est possible dans la grille par rapport
    * a ce qu'elle contient deja.
    *
    * @param x position x dans la grille
    * @param y position y dans la grille
    * @param value
    * valeur a mettre dans la case
    * @throws IllegalArgumentException si x ou y sont hors bornes (0-8)
    * @throws IllegalArgumentException si value n'est pas ['1',...,'9',..]
    * @return true si value est ('1',...,'9',..)
    */
    boolean possibleValues(int x, int y, char value) {
		return false;
	}
}
